...
cat<<-EOF
    Universe = vanilla 
    Executable = cmssw.sh
    Arguments = $codegen_exe $card_name $card_dir
    
    Error = condor_log/job.err.\$(Cluster)-\$(Process) 
    Output = condor_log/job.out.\$(Cluster)-\$(Process) 
    Log = condor_log/job.log.\$(Cluster) 
    
    #transfer_input_files = $codegen_exe, $input_files, gridpack_generation.sh, /usr/bin/unzip
...
stash_tmpdir=$(mktemp -d --tmpdir=/tmp/$USER)
